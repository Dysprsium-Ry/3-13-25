...
{
    public partial class BienvenidosOnlineTutoringService : Form
    {
        ToolTip toolTip = new ToolTip();

        public BienvenidosOnlineTutoringService()
        {
            InitializeComponent();
            refresh();
            OverallTotalFeeUpdate();
            dateTimePickerReservationDate.MinDate = DateTime.Now;
            dateTimePickerScheduledDate.MinDate = dateTimePickerReservationDate.Value;
        }
        ...
        //        dateTimePickerReservationDate.Value = endTime;
        //        return endTime;
        //    }
        //    dateTimePickerReservationDate.Value = date;
        //    return date;
        //}
        public DateTime CalculateAdjustedDateTime(DateTime startDate, int hoursToAdd)
        {
            ...
            while (hoursToAdd > 0)
            {
                ...
                {
                    if (remainingTimeToday.TotalHours >= hoursToAdd)
                    {
                        result = result.AddHours(hoursToAdd);
                        break;
                    }
                    else
                    {
                        result = result.Add(remainingTimeToday);
                        hoursToAdd -= (int)remainingTimeToday.TotalHours;
                    }
                }
        ...
        private void buttonBookService_Click(object sender, EventArgs e)
        {
            ...
            if (AdditionalBooking.TransactionsList.Count > 0)
            {
                ...
                foreach (var transaction in AdditionalBooking.TransactionsList)
                {
                    ...
                    ServiceBooking.CalculatedDate = transaction.ScheduledDate;
                    ...
                    else { ServicesClass.ServiceBooking(); MessageBox.Show($"Service {ServiceBooking.Service} is Booked Succesfully", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Information); }
                }
                if (!isDateBooked) { AdditionalBooking.TransactionsList.Clear(); dataGridView.DataSource = null; refresh(); }
            }
            else
            {
                ...
                ServiceBooking.CalculatedDate = dateTimePickerScheduledDate.Value;
                ...
                else { ServicesClass.ServiceBooking(); ServicesClass.ToolDecrement(); MessageBox.Show("Booked Succesfully", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Information); refresh(); }
            }
        }
        ...
        private void textBoxTotalFee_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(TextBox_ServiceDuration.Text, out int hoursToAdd))
            {
                DateTime startDate = dateTimePickerReservationDate.Value;

                DateTime adjustedDateTime = CalculateAdjustedDateTime(startDate, hoursToAdd);

                dateTimePickerScheduledDate.Value = adjustedDateTime;
            }
        }
        ...
        private void buttonAddAdditionalServiceLib_Click(object sender, EventArgs e)
        {
            ...
            var newbooking = new Booking
            {
                ...
                TotalFee = totalFee,
                ScheduledDate = dateTimePickerScheduledDate.Value.Date
            };
        ...
        private void HandleInputDisplay(KeyPressEventArgs e)
        {
            ...
            {
                TextBox_ServiceDuration.Text = "0";
                dateTimePickerScheduledDate.Value = dateTimePickerReservationDate.Value;
                e.Handled = true;
                return;
            }
        ...
        private void comboBoxInventoryService_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            foreach (Control control in tabPageInventory.Controls)
            {
                if (control is TextBox textbox && textbox.Enabled == true)
                {
                    textbox.Clear();
                }
            }
